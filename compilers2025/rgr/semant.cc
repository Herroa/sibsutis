...
bool SemanticAnalyzer::is_subtype(Symbol child, Symbol parent)
{
    ...
    {
        if (parent_map.find(curr) == parent_map.end())
            break;
...
Symbol SemanticAnalyzer::type_check_expr(Expression expr, Class_ cls)
{
    ...
    if (auto obj = dynamic_cast<object_class *>(expr))
    {
        ...
        {
            semant_error(cls, (std::string("Undeclared identifier: ") + name->get_string()).c_str());
            return idtable.add_string("Object");
        }
    ...
    if (auto dispatch = dynamic_cast<dispatch_class *>(expr))
    {
        ...
        Symbol class_to_check = expr_type == idtable.add_string("SELF_TYPE") ? cls->get_name() : expr_type;

        if (class_map.find(class_to_check) == class_map.end())
        {
            semant_error(cls, (std::string("Dispatch on undefined class") + class_to_check->get_string()).c_str());
            return idtable.add_string("Object");
        }
        ...
        if (!found_method)
        {
            semant_error(cls, (std::string("Method ") + method_name->get_string() +
                               std::string(" not found in class ") + class_to_check->get_string())
                                  .c_str());
            return idtable.add_string("Object");
        }
        ...
        {
            semant_error(
                cls, (std::string("Wrong number of arguments passed to method ") + method_name->get_string()).c_str());
            return idtable.add_string("Object");
        }
        ...
        for (int i = 0; i < args->len(); ++i)
        {
            ...
            {
                semant_error(cls, "Type of Argument does not conform to declared parameter type.");
            }
        }
...
void SemanticAnalyzer::check_method_body(method_class *method, Class_ cls)
{
    ...
    for (int i = formals->first(); formals->more(i); i = formals->next(i))
    {
        ...
        {
            semant_error(
                cls, std::string("Parameter redeclares existing variable '" + std::string(name->get_string()) + "'.")
                         .c_str());
        }
    ...
    {
        semant_error(cls, "Return type of method body does not conform to declared return type.");
    }
...
void SemanticAnalyzer::check_method_params(method_class *method, Class_ cls)
{
    ...
    for (int i = formals->first(); formals->more(i); i = formals->next(i))
    {
        ...
        {
            semant_error(cls, ("Duplicate method parameter name '" + std::string(fname->get_string()) + "'.").c_str());
        }
        else
        {
...
void SemanticAnalyzer::check_class(Class_ cls)
{
    ...
    for (int i = features->first(); features->more(i); i = features->next(i))
    {
        ...
        if (dynamic_cast<method_class *>(f))
        {
            ...
            {
                semant_error(cls, ("Duplicate method name '" + std::string(fname->get_string()) + "' in class: '" +
        ...
        else if (dynamic_cast<attr_class *>(f))
        {
            ...
            if (feature_names.count(fname->get_string()))
            {
                semant_error(cls, ("Duplicate attribute name '" + std::string(fname->get_string()) + "' in class: '" +
                                   std::string(cls->get_name()->get_string()) + "'")
                                      .c_str());
            }
            else
...
void SemanticAnalyzer::analyze()
{
    ...
    for (int i = classes->first(); classes->more(i); i = classes->next(i))
    {
        ...
        {
            semant_error(
                cls, std::string("Class '" + std::string(name->get_string()) + "' is defined more than once.").c_str());
            continue;
        }
        else
        {
        ...
        if (name == idtable.add_string("Main"))
            has_main = true;
    ...
    {
        if (class_map.find(parent) == class_map.end() && parent != idtable.add_string("Object"))
        {
            semant_error(class_map[name],
                         std::string("Parent class of '" + std::string(name->get_string()) + "' not defined.").c_str());
        }
    }
    ...
    for (auto &[name, _] : class_map)
    {
        if (has_inheritance_cycle(name))
        {
            semant_error(
                class_map[name],
                std::string("Inheritance cycle detected at class '" + std::string(name->get_string()) + "'.").c_str());
        }
    }
